...
cd.stats_1 <- cd[, rbindlist(lapply(names(.SD), 
                 ...
                 by = c('bz_key','ks_key','cd_key','year'),
                 .SDcols=c("zyts","zfy","ypf","qtf") ]
...
#library(RODBC)
conn=odbcConnect("",uid="",pwd="")
sqlSave(conn, cd.stats_2, 
        "FACT_CD_BZKS_VALUE_Y",
        safer=FALSE,append=TRUE,
        rownames=FALSE,varTypes=c(year="Date"))
odbcClose(conn)
