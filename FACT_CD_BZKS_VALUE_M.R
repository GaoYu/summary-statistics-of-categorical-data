...
conn <- odbcConnect("et_hospital_cubedb",uid="sa",pwd="biadmin")
...
#病种科室年度住院天数、总费用、药费、其他费等的最大值，最小值等计算
...
cd.stats_1 <- cd[, rbindlist(lapply(names(.SD), 
                 ...
                 by = c('bz_key','ks_key','cd_key','month'),
                 .SDcols=c("zyts","zfy","ypf","qtf") ]
...
#library(RODBC)
conn=odbcConnect("et_hospital_cubedb",uid="sa",pwd="biadmin")
sqlSave(conn, cd.stats_2, "FACT_CD_BZKS_VALUE_M",safer=FALSE,append=TRUE,rownames=FALSE)
odbcClose(conn)
